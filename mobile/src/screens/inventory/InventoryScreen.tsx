...
import { Appear, Button, Card, EmptyState, H1, Pill, SkeletonRows } from '../../components/ui';
...
export default function InventoryScreen() {
  ...
  const { data = [], isLoading } = useQuery({ queryKey: qk.inventory(), queryFn: listInventory });
  ...
  const refresh = () => invalidate(qc, 'inventory');
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }} edges={['top']}>
      <View className="px-5 pt-2 pb-3">
        <View className="flex-row items-center justify-between">
          <H1>Inventory</H1>
          {isAdmin && isDesktop && (
            <Pressable onPress={() => setShowAdd(true)} className="flex-row items-center bg-forest-600 rounded-xl px-4 py-2.5 active:opacity-80 web:hover:bg-forest-700 web:transition-colors">
              <Ionicons name="add" size={16} color="#fff" />
              <Text className="text-white font-semibold text-sm ml-1.5">Add item</Text>
            </Pressable>
          )}
        </View>
        {/* Filter chips */}
        <View className="flex-row gap-2 mt-4">
          <Pressable onPress={() => setLowOnly(false)} className={`px-4 py-2 rounded-full web:transition-colors ${!lowOnly ? 'bg-forest-600' : 'bg-white border border-line'}`}>
            <Text className={`text-sm font-semibold ${!lowOnly ? 'text-white' : 'text-muted'}`}>All ({data.length})</Text>
          </Pressable>
          <Pressable onPress={() => setLowOnly(true)} className={`flex-row items-center px-4 py-2 rounded-full web:transition-colors ${lowOnly ? 'bg-danger' : 'bg-white border border-line'}`}>
      ...
      {isLoading ? (
        <View className="px-5"><SkeletonRows count={5} /></View>
      ) : (
        <FlatList
          data={shown}
          keyExtractor={(i) => i.id}
            ...
            return (
              <Appear delay={Math.min(index, 10) * 45}>
                <Pressable disabled={!isAdmin} onPress={() => setAdjust(item)}>
                  <Card className={`mb-3 ${low ? 'border-red-200' : ''}`}>
          ...
          ListEmptyComponent={
            <EmptyState
              icon="cube-outline"
              title={lowOnly ? 'All stock levels are fine' : 'No inventory items'}
              hint={lowOnly ? 'Nothing needs reordering right now.' : 'Add your first item to start tracking stock.'}
            />
          }
        />
      )}
...
function AdjustSheet({ item, onClose, onDone }: { item: InventoryItem; onClose: () => void; onDone: () => void }) {
  ...
  return (
    <Sheet title={item.name} subtitle={`On hand: ${item.quantity} ${item.unit}`} onClose={onClose}>
      <View className="flex-row items-center bg-slate-50 rounded-xl px-3 border border-line mb-4">
...
// Exported so ServicesScreen, SettingsScreen, ExpensesScreen can reuse
  ...
  return (
    <Modal transparent animationType="slide" onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        ...
        <View className="bg-white rounded-t-3xl px-5 pt-4 pb-8" style={{ borderTopWidth: 1, borderTopColor: colors.line }}>
...
// Exported so other screens can reuse this input style
export function Field(props: any) {
  return (
    <TextInput
      {...props}
      placeholderTextColor={colors.muted}
      className="bg-slate-50 rounded-xl px-3 py-3 text-ink border border-line mb-3"
    />
  );
}
