...
import { Avatar, Card, EmptyState, IconButton, Pill, SkeletonRows } from '../../components/ui';
...
export default function StaffScreen({ navigation }: any) {
  ...
  const { data = [], isLoading } = useQuery({ queryKey: qk.staff(), queryFn: listStaff });

  const refresh = () => invalidate(qc, 'staff');
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'transparent' }} edges={['top']}>
      <View className="flex-row items-center justify-between px-5 py-3">
        ...
        <Text style={{ fontFamily: 'Inter_700Bold', fontSize: 16 }} className="text-ink">Staff & Accounts</Text>
        <IconButton name="person-add-outline" onPress={() => navigation.navigate('CreateStaff', { role: 'DOCTOR' })} color={colors.forest[600]} />
      </View>

      {isLoading ? (
        <View className="px-5"><SkeletonRows count={4} /></View>
      ) : (
        <FlatList
          data={data}
          keyExtractor={(s) => s.id}
            ...
            <Card className={`mb-3 flex-row items-center ${item.is_active ? '' : 'opacity-60'}`}>
              ...
              <View className="flex-1 ml-3">
                <View className="flex-row items-center">
                  ...
                  {item.id === profile?.id && <Text className="text-[10px] text-forest-600 ml-2 font-semibold bg-forest-50 px-1.5 py-0.5 rounded-full">YOU</Text>}
                </View>
          ...
          ListEmptyComponent={
            <EmptyState icon="people-outline" title="No staff yet" hint="Add your first team member to get started." />
          }
        />
      )}
